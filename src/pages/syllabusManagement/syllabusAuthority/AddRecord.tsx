...
import { useAddSyllabusAuthority } from "@/hooks/apis/useSyllabusAuthority";

function AddRecord({ open, handleClose }: TAddRecord) {
  const [form] = Form.useForm();
  const [btnClicked, setBtnClicked] = useState(false);
  const { mutate, isLoading, isSuccess } = useAddSyllabusAuthority();

  const onFinish = (values: TSyllabusAuthorityFormDataOnAdd) => mutate(values);
